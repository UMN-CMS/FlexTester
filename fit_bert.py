...
class FitData:
    ...
    def do_fit(self, scan_idx, scan):
        ...
        if len(maxes) < 2:
            ...
            self.conn.send("Unstable scan with many maxima for index {}, check connection".format(scan_idx))
            return {"Eye Opening": -999, "Midpoint": -999, "Midpoint Errors": -999}
    ...
    def plot_scan(self, scan, scan_idx, fit_params):
        ...
        return {"Eye Opening": int(round(x2-x1)), "Midpoint": int(round((x2+x1)/2)), "Midpoint Errors": int(mid_err)}
