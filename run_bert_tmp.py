...
#        print("Starting in run_bert_tmp. Inside of __init__ in the BERT class")
...
#        print("**kwargs:, output='BERT.csv', iskip=5, nbits=1e8, module=None, clock=True")
        self.scans = []
        self.crossovers = []
#        print("Making an instance of the wagon class in Wagoneer.py. Leaving run_bert_tmp.py and going to the Wagon class in wagoneer.py\n")
        self.wagon = Wagon()
#        print("Back in the bert method in run_bert_tmp.py")
        self.mod = kwargs['module']
        self.clock = kwargs['clock']
        self.iskip = kwargs['iskip']
#        print("Variables in bert method:")
#        print("self.mod:", kwargs['module'])
#        print("self.clock:", kwargs['clock']) 
#        print("self.iskip:", kwargs['iskip'])
#        self.invert_map = [0,1,0,0,1,1,1,1,1]
        self.invert_map = [1,1,0,0,1,0,1,1,1]
#        print("Calling the self.reset_zeros() method in run_bert_tmp.py")
        self.reset_zeros()
        ...
        self.set_inverts()
        #self.set_prbs_len()
#        print("Calling the self.set_prbs(1) method from the bert method in run_bert_tmp.py")
        self.set_prbs(1)
#        print(self.set_prbs(1))
#        print("Calling the self.run_long_scan(kwargs['iskip'], kwargs['nbits'], kwargs['output']) method from the bert method in run_bert_tmp.py")
...
# This version for BH
#        self.scan_mask = [False, True, True, True, True, True, False, False, False, False, False]
# This version for FH
        self.scan_mask = [False, True, True, True, True, True, True, True, False, False, False]
...
#        self.conn.send("LCD ; Passed:{} Test:4".format(self.passed))
        time.sleep(0.1)
        self.conn.send("Done.")
#        print("Returning self.passed and self.data")
        return self.passed, self.data

    def reset_zeros(self):
#        print("Inside of reset_zeros in run_bert_tmp.py")
        ZERO_MODE = 7
#        print("Going back to wagoneer.py to run self.wagon.set_tx_mode(i, ZERO_MODE), where i loops from 0 to 4 and ZERO_MODE = 7\n")
        for i in range(0,self.wagon.ntx):
            self.wagon.set_tx_mode(i, ZERO_MODE)
#        print("Leaving wagoneer.py and going back to run_bert_tmp.py\n")

    def set_prbs_len(self):
        self.wagon.set_prbs_len(30000000)

    def set_prbs(self, tx):
#        print("Inside of set_prbs in run_bert_tmp.py")
#        print("Going back to wagoneer.py to run self.wagon.set_tx_mode(i, PRBS), where i loops from 0 to 4 and PRBS = 1\n")
        PRBS = 1
        for i in range(0,self.wagon.ntx):
...
#        print("Inside of run_long_scan(self, iskip=1, max_bit=1e8, output="") method in run_bert_tmp.py")
#        print("iskip:", iskip)
...
#            print("Leaving run_bert_tmp.py and going to the scan method in wagooner.py.\n")
